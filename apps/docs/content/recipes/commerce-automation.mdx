...
This document guides you on how you can implement different forms of commerce automation within your InBzar project.
...
InBzar provides the necessary architecture and resources to implement commerce automation related to order management, customer service, and more. It mainly utilizes an event bus that allows you to listen to specific event triggers and perform asynchronous actions.
...
InBzar already provides a [Re-stock notifications plugin](../plugins/other/restock-notifications.md). You can also implement this by creating custom entities, triggering custom events, and handling these events with a subscriber.
...
<DocCardList colSize={6} items={[
  ...
  },
  {
    ...
    customProps: {
      ...
      description: 'Learn how to create a custom entity in the InBzar backend.',
    }
  },
  {
    type: 'link',
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Learn how to emit a custom event in the InBzar backend.',
    }
  },
  {
    type: 'link',
...
You can use InBzar's Notification Service to handle notifications triggered by actions on customer orders or profiles.
...
The [Event reference](../development/events/events-list.md) includes an extensive list of events triggered by the InBzar core.

InBzar also provides official notification plugins that integrate with third-party services, such as [SendGrid](../plugins/notifications/sendgrid.mdx) or [TwilioSMS](../plugins/notifications/twilio-sms.md).
...
You can implement automatic synchronization in InBzar using scheduled jobs. A scheduled job runs at a specified date and time interval in the background of your InBzar backend. Within the scheduled job, you can synchronize your internal or external data.
...
<DocCard item={{
  ...
  customProps: {
    icon: Icons['academic-cap-solid'],
    description: 'Learn how to create a scheduled job in InBzar.',
  }
}} />
...
Using InBzar's architecture and commerce features, you can automate a large amount of order management functionalities.
...
<DocCardList colSize={4} items={[
  {
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Learn about the event architecture system in the InBzar backend.',
    }
  },
  {
    type: 'link',
  ...
  },
  {
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Check out triggered events in InBzar and their expected payloads.',
    }
  },
]} />
...
InBzar's commerce features are geared towards automating RMA flows and ensuring a good customer experience.
...
InBzar also allows the store operator to edit orders, receive customer approval for the edits, and authorize additional payment if necessary, all within a seamless workflow.
...
<DocCardList colSize={4} items={[
  ...
  },
  {
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Learn about events in InBzar and how to listen to events.',
    }
  },
  {
    type: 'link',
...
For example, you may send a newsletter when new products are added to your store. In InBzar, this can be handled by listening to the [product events](../development/events/events-list.md#product-events), such as `product.created`, using a Subscriber, then sending an email to subscribed customers with tools like [SendGrid](../plugins/notifications/sendgrid.mdx) or [Mailchimp](../plugins/notifications/mailchimp.md).
...
The use cases mentioned in this guide exemplify what commerce automation you can perform or implement with InBzar. InBzar provides the necessary development tools and mechanisms to automate tasks and manual work.
