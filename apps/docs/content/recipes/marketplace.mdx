...
This document guides you through the different documentation resources that will help you build a marketplace with InBzar.
...
By default, entities like users, products, or orders aren't associated with a store, as it's assumed there's one store in InBzar. For a marketplace, each of these entities should be associated with their respective stores.

To associate these entities with the `Store` entity, you need to extend and customize entities created in the InBzar core package `@medusajs/medusa`, such as the `User` entity, to add a relation to the `Store` entity.
...
After associating entities with stores, you'll need to customize how certain data management functionalities are implemented in the InBzar core package.
...
<DocCard item={{
  ...
  customProps: {
    icon: Icons['academic-cap-solid'],
    description: 'Learn how to extend a service in InBzar',
  }
}} />
...
Payment and fulfillment providers can be added through plugins or directly in your project. You can either create your own provider, use one of InBzar's official plugins, or use community plugins.
...
<DocCardList colSize={6} items={[
  {
    ...
    label: 'Install an Official InBzar Plugin',
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Check out available InBzar plugins to install.',
    }
  },
  {
    type: 'link',
...
<DocCard item={{
  ...
  customProps: {
    icon: Icons['academic-cap-solid'],
    description: 'Learn how to implement role-based access control (RBAC) in InBzar.',
  }
}} />
...
Our documentation includes deployment guides for a basic InBzar backend. You should be able to follow it to deploy your customized marketplace, as well.
...
You can find other resources for your marketplace development in the [InBzar Development section](../development/overview.mdx) of this documentation.
