...
This document guides you through the different documentation resources that will help you build subscription-based purchasing in InBzar.
...
Recommended read: [How Goodchef built subscription-based purchases with InBzar](https://medusajs.com/blog/goodchef-webbers-powering-subscriptions-with-medusas-commerce-modules/).
...
If you want to store the subscription details in a new table in the database, you can do that by creating an entity. If you want to extend an existing entity in InBzar's core, such as the `Order` entity, to add details related to the subscription, you can extend an entity.
...
<DocCardList colSize={6} items={[
  {
    ...
    customProps: {
      ...
      description: 'Learn how to create an entity in InBzar.',
    }
  },
  {
    type: 'link',
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Learn how to extend an entity in InBzar.',
    }
  },
]} />
...
There are different ways to implement subscriptions in your InBzar backend. This document discusses two options: using Stripe subscriptions, or implementing subscriptions logic within the backend, independent of a specific payment provider.
...
InBzar provides a Stripe plugin, however, it doesn't handle subscriptions. You can either use that plugin to add the subscription feature on top of it, or create a custom Stripe Subscription payment provider.

<DocCardList colSize={6} items={[
  {
    type: 'link',
    href: 'https://github.com/medusajs/medusa/tree/v1.x/packages/medusa-payment-stripe',
    label: "Use InBzar's Stripe Plugin",
    customProps: {
      icon: Icons['github'],
      description: "Check out InBzar's stripe plugin to build subscription on top of it.",
    }
  },
  {
    type: 'link',
...
<DocCardList colSize={6} items={[
  {
    ...
    customProps: {
      icon: Icons['academic-cap-solid'],
      description: 'Learn how to create a subscriber in InBzar.',
    }
  },
  {
    type: 'link',
...
InBzar's [admin plugin](../admin/quickstart.mdx) can be extended to add widgets or new blocks to existing pages, add UI routes to add new pages, or add setting pages.
...
InBzar provides a Next.js Starter Template that you can use with InBzar. Since you've customized your InBzar project, you'll need to either customize the existing Next.js Starter Template, or create a custom storefront.
...
Our documentation includes deployment guides for a basic InBzar backend. You should be able to follow it to deploy your customized backend, as well.
...
You can find other resources for your development in the [InBzar Development section](../development/overview.mdx) of this documentation.
