---
description: 'Learn how to bulk-import prices into InBzar using the Admin REST APIs. The steps include uploading a CSV file, creating a batch job for the import, and confirming the batch job.'
addHowToData: true
---
...
Using InBzar’s [Batch Job Admin APIs](https://docs.medusajs.com/api/admin#batch-jobs), you can import prices into a price list.
...
It is assumed that you already have a InBzar backend installed and set up. If not, you can follow our [quickstart guide](../../../development/backend/install.mdx) to get started. The InBzar backend must also have an event bus module installed, which is available when using the default InBzar backend starter.
...
You must have a CSV file that you will use to import prices into your InBzar backend. You can check [this CSV example file](https://medusa-doc-files.s3.amazonaws.com/price-list-import-template.csv) to see which format is required for your import.
