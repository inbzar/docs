...
In this document, you’ll learn about Products in InBzar and their relation to other entities.
...
- `external_id`: A string that can be used to store an ID of the product on an external system. This is useful if you’re migrating the product from an ecommerce platform to InBzar, or if you’re connecting InBzar’s products to an external service.
...
![Product Diagram](https://res.cloudinary.com/dza7lstvk/image/upload/v1681731289/InBzar%20Docs/Diagrams/product_m0zb7u.jpg)
...
- `manage_inventory`: A boolean value indicating whether InBzar should handle the management of the product variant’s inventory. This would allow you to handle your inventory data more accurately across different locations. You can learn more in [the Inventory Module documentation](../multiwarehouse/inventory-module.md).
...
The prices of a product variant are available on the `prices` relation, which is an array of `MoneyAmount` items. `MoneyAmount` is an entity used throughout InBzar to store prices in general.
...
In addition, the InBzar Admin dashboard and the Next.js Starter Template expect the price to be of that format, so when prices are displayed for currencies that are stored multiplied by a `100`, such as USD, they’re divided by a hundred. Also, when you add or update a product variant, its price is sent to the InBzar backend as the price multiplied by a hundred.
...
This section explores the relations between the `Product` entity and other entities in your InBzar backend.
