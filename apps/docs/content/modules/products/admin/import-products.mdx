---
description: 'Learn how to import products into InBzar using the Admin REST APIs. The steps include uploading a CSV file, creating a batch job for the import, and confirming the batch job.'
addHowToData: true
---
...
In this document, you’ll learn how to use the Admin APIs to bulk import products into a InBzar backend.

## Overview

Using InBzar’s [Batch Job Admin APIs](https://docs.medusajs.com/api/admin#batch-jobs_getbatchjobs), you can import products into your InBzar backend. This will either add new products or update existing products.
...
It is assumed that you already have a InBzar backend installed and set up. If not, you can follow the [quickstart guide](../../../development/backend/install.mdx) to get started. The InBzar backend must also have an event bus module installed, which is available when using the default InBzar backend starter.
...
You must have a CSV file that you will use to import products into your InBzar backend. You can check [this CSV example file](https://medusa-doc-files.s3.amazonaws.com/product-import-sales-channels.csv) to see which format is required for your import.
...
This guide includes code snippets to send requests to your InBzar backend using InBzar’s JS Client, among other methods.
