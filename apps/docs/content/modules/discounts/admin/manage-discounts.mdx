---
description: 'Learn how to implement discount functionalities for admins in InBzar using the REST APIs. This includes creating, updating, and deleting discounts, and managing conditions of discounts.'
addHowToData: true
---
...
Using InBzar’s Discount Admin APIs, you can manage discounts, their conditions, status, rules, and more. You can also manage [dynamic discounts and their discount codes](https://docs.medusajs.com/api/admin#discounts_postdiscountsdiscountdynamiccodes).
...
You can use InBzar’s Admin APIs to achieve more functionalities as well. Check out the [API reference](https://docs.medusajs.com/api/admin#discounts) to learn more.
...
It is assumed that you already have a InBzar backend installed and set up. If not, you can follow the [quickstart guide](../../../development/backend/install.mdx) to get started.

### JS Client

This guide includes code snippets to send requests to your InBzar backend using InBzar’s JS Client, JavaScript’s Fetch API, or cURL.

If you follow the JS Client code blocks, it’s assumed you already have [InBzar’s JS Client](../../../js-client/overview.mdx) installed and have [created an instance of the client](../../../js-client/overview.mdx#configuration).

### InBzar React

This guide also includes code snippets to send requests to your InBzar backend using InBzar React, among other methods.

If you follow the InBzar React code blocks, it's assumed you already have [InBzar React installed](../../../medusa-react/overview.mdx) and have [used MedusaProvider higher in your component tree](../../../medusa-react/overview.mdx#usage).
...
<Tabs groupId="request-type" isCodeTabs={true}>
  <TabItem value="client" label="InBzar JS Client" default>
  ...
  <TabItem value="medusa-react" label="InBzar React">
...
- `rule`: This parameter is required. It is an object having at least the following fields:
    - `type`: A string indicating the type of discount. It can be `fixed`, `percentage`, or `free_shipping`. When using the InBzar JS Client, you must use the enum type [DiscountRuleType](../../../references/entities/enums/entities.DiscountRuleType.mdx) for the value.
    ...
    - `allocation`: A string indicating how the discount should be applied. Can be `item` or `total`. If the type is not `fixed`, then this has no effect. When using the InBzar JS Client, you must use the enum type [AllocationType](../../../references/entities/enums/entities.AllocationType.mdx) for the value.
...
- `in` indicates that the discount should be applied to the specified resources in this condition. When using the InBzar JS Client, `DiscountConditionOperator.IN` must be used as the value.
- `not_in` indicates that the discount should be applied to all resources except those specified in this condition. When using the InBzar JS Client, `DiscountConditionOperator.NOT_IN` must be used as the value.
